...
description: Generates a comprehensive threat_model.md document for the current project using the STRIDE methodology. Use this skill when the user asks to create, generate, build, write, or update a threat model for the repository or project.
license: MIT
---

# Threat Model Generator

This skill analyzes the current project and produces a `threat_model.md` file in
the repository root containing a thorough threat model using the **STRIDE**
methodology (Spoofing, Tampering, Repudiation, Information Disclosure, Denial
...
Inspect the working directory to understand what the project is. Gather as much
of the following as possible before writing the model:
...
2. **Dependencies** — List direct dependencies from manifest files. Flag any
   that are known to commonly be security-sensitive (auth, crypto, networking,
...
6. **Existing security controls** — Note presence of:
   ...
   - Dependency scanning, SAST, secret scanning configs (e.g., Dependabot,
...
| Impact | Confidentiality / Integrity / Availability impact, severity (Low/Med/High/Critical) |
...
Write the output to a file named `threat_model.md` at the repository root,
**overwriting** any existing file with that name. Use the following structure:

```markdown
# Threat Model: <Project Name>

> Generated by the `threat-model-generator` Copilot skill on <YYYY-MM-DD>.
...
- Data flow summary
- ASCII or Mermaid diagram of the system and trust boundaries
...
## 3. Assets
Bulleted list of assets worth protecting (user data, credentials, signing keys,
intellectual property, availability, etc.) with sensitivity ratings.
...
- **Be actionable.** Recommendations should be concrete (e.g., "Add
  `helmet` middleware in `src/server.ts`") rather than vague ("Improve
  security headers").
